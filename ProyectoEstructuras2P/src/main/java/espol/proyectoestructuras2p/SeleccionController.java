...
public class SeleccionController {
    ...
    protected void setJRadio() {
        if (!jugadorRadio.isSelected()) { jugadorRadio.setSelected(true); cpuRadio.setSelected(false); }
        else if (cpuRadio.isSelected()) { cpuRadio.setSelected(false); jugadorRadio.setSelected(true); }
        setJugadorEmpieza(true);
    }
    
    @FXML
    protected void setCRadio() {
        if (!cpuRadio.isSelected()) { cpuRadio.setSelected(true); jugadorRadio.setSelected(false); }
        else if (jugadorRadio.isSelected()) { jugadorRadio.setSelected(false); cpuRadio.setSelected(true); }
        setJugadorEmpieza(false);
    }

    @FXML
    protected void setXRadio() {
        if (!xRadio.isSelected()) { xRadio.setSelected(true); oRadio.setSelected(false); }
        else if (oRadio.isSelected()) { oRadio.setSelected(false); xRadio.setSelected(true); }
        setFiguraJugador('X');
    }
    
    @FXML
    protected void setORadio() {
        if (!oRadio.isSelected()) { oRadio.setSelected(true); xRadio.setSelected(false); }
        else if (xRadio.isSelected()) { xRadio.setSelected(false); oRadio.setSelected(true); }
        setFiguraJugador('O');
    }
